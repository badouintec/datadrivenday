 ...
 * Uses pdf-lib loaded from CDN to build the certificate entirely in the browser,
 * avoiding Cloudflare Workers CPU limits.
 */

const PDF_LIB_CDN = 'https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js';

// Site constants (mirrored from src/data/site.ts to avoid server dependency)
const SITE = {
...
function fitHeadline(name, limit) {
  if (limit === undefined) limit = 28;
  var words = sanitizeText(name).split(' ');
  var lines = [];
  var current = '';
...
/** Load pdf-lib from CDN (cached after first load) */
function loadPdfLib() {
  ...
  return new Promise(function (resolve, reject) {
    ...
    script.onerror = function () { reject(new Error('No se pudo cargar pdf-lib')); };
    document.head.appendChild(script);
  });
}

/**
 * Generate and download the recognition PDF entirely in the browser.
 * @param {Object} participant - participant data from state
 */
async function generateRecognitionPdf(participant) {
  var lib = await loadPdfLib();
  var rgb = lib.rgb;
  var StandardFonts = lib.StandardFonts;
  ...
  var pdf = await lib.PDFDocument.create();
  ...
  var page = pdf.addPage([842, 595]);
  var W = page.getWidth();
  var H = page.getHeight();
  var M = 40;

  var helvetica = await pdf.embedFont(StandardFonts.Helvetica);
  var helveticaBold = await pdf.embedFont(StandardFonts.HelveticaBold);
  ...
  for (var ci = 0; ci < corners.length; ci++) {
    page.drawCircle({ x: corners[ci].x, y: corners[ci].y, size: cornerSize, color: COLORS.accentBlue, opacity: 0.5 });
    page.drawCircle({ x: corners[ci].x, y: corners[ci].y, size: cornerSize - 2, color: COLORS.accentSky, opacity: 0.8 });
  }
  ...
  // ── Embed PNG logo (fast in browser, no CPU limits) ─────────────────────
  var logoBytes = base64ToBytes(LOGO_BASE64);
  var logoImage = await pdf.embedPng(logoBytes);
  var logoScale = 0.14;
  var logoW = 900 * logoScale;
  var logoH = 320 * logoScale;
  ...
  var folio = sanitizeText(participant.recognitionFolio || 'DDD-2026-' + participant.id.slice(0, 8).toUpperCase());
  ...
  page.drawText(SITE.tagline, { x: M + 16, y: footerY + 17, size: 8.5, font: helvetica, color: COLORS.textMuted, maxWidth: W - M * 2 - 32 });
...
// Expose globally for participant-dashboard.js
window.generateRecognitionPdf = generateRecognitionPdf;
