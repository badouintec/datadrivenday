...
export async function generateRecognitionPdf(participant: {
  ...
  // Vector logo mark — no PNG decoding needed, renders instantly
  const logoX = M + 16;
  const logoY = headerY + 22;
  const logoBoxW = 60;
  const logoBoxH = 32;
  page.drawRectangle({ x: logoX, y: logoY, width: logoBoxW, height: logoBoxH, color: COLORS.surface2 });
  page.drawRectangle({ x: logoX, y: logoY + logoBoxH - 3, width: logoBoxW, height: 3, color: COLORS.accentBlue, opacity: 0.9 });
  page.drawText('DATA', { x: logoX + 4, y: logoY + logoBoxH - 14, size: 9, font: helveticaBold, color: COLORS.accentSky });
  page.drawText('DRIVEN', { x: logoX + 4, y: logoY + logoBoxH - 24, size: 7, font: helvetica, color: COLORS.textMuted });
  page.drawText('DAY', { x: logoX + 4, y: logoY + 5, size: 9, font: helveticaBold, color: COLORS.accentBlue });
